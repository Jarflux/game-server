...
client.on('connect', function(connection) {
    ...
    const data = {
        action:'join',
        data: 'Anne', //'Client ' + Math.floor(Math.random()*100)
        api_key: 'wjAv9bvZl)"IbB`1OI^%ZJa+SAXA%$'
    };
            ...
            switch (message.action) {
                ...
                case 'your_turn':
                    ...
                    if (option.action === 'fold' || option.action === 'call' || option.action === 'check') {
                        ...
                        let raise = Math.floor(randomIntFromInterval(option.minimum, option.maximum) / 5);
